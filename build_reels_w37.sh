...
REELS_DIR="/Users/morrislin/mw-social-assets/MORENE/W37/Reels"
...
for ID in "${IDS[@]}"; do
  case "$ID" in
    R1) TRACK="calm" ;;        # 8/20 夏轉秋換季過渡
    R2) TRACK="contempl" ;;    # 8/23 處暑樹脂調
    R3) TRACK="bright" ;;      # 8/26 前中後調科普
  esac
  AUDIO="${AUDIO_DIR}/MORENE_reels_${TRACK}.mp3"
  OUT="${OUT_DIR}/MORENE_W37_${ID}.mp4"
  echo ">>> Building W38_${ID} [${TRACK}]..."

  ffmpeg -y \
    -loop 1 -t 3.0  -i "${REELS_DIR}/MORENE_W37_${ID}_f1.png" \
    -loop 1 -t 4.8  -i "${REELS_DIR}/MORENE_W37_${ID}_f2.png" \
    -loop 1 -t 4.8  -i "${REELS_DIR}/MORENE_W37_${ID}_f3.png" \
    -loop 1 -t 4.8  -i "${REELS_DIR}/MORENE_W37_${ID}_f4.png" \
    -loop 1 -t 4.8  -i "${REELS_DIR}/MORENE_W37_${ID}_f5.png" \
    -loop 1 -t 4.8  -i "${REELS_DIR}/MORENE_W37_${ID}_f6.png" \
    -i "$AUDIO" \
    -filter_complex "
